...
function [kinetic_params, estimated_concentration] = Simplex_Fit_Kinetic_Tofts(observed_concentration, gd_AIF, initial_params, FR_mins, integration_method, fitting_method)
...
if strcmp(fitting_method, 'simplex')
...
function cost = kinetic_fit(params)
    ...
    if strcmp(integration_method, 'recursive')
    ...
    res = exp(-kep.*t);
    estimated_concentration = ktrans.*conv(gd_AIF, res).*FR_mins;
